   ...
   <div class="blog_listing_image">
    ...
    <div class="prop-info">
        ...
        <div class="info-container">
            <a  data-original-title="<?php _e('Edit sublease','wpestate');?>"   class="dashboad-tooltip" href="<?php  print $edit_link;?>"><i class="fa fa-pencil-square-o editprop"></i></a>
            <a  data-original-title="<?php _e('Delete sublease','wpestate');?>" class="dashboad-tooltip" onclick="return confirm(' <?php echo __('Are you sure you wish to delete ','wpestate').get_the_title(); ?>?')" href="<?php print esc_url_raw(add_query_arg( 'delete_id', $post_id,  $_SERVER['REQUEST_URI']  ) );?>"><i class="fa fa-times deleteprop"></i></a>  
            ...
            <?php }else{ ?>
                <a  data-original-title="<?php _e('Item has 0 views','wpestate');?>"   class="dashboad-tooltip show_statsx"  data-listingid="<?php echo $post_id;?>" href="#"><i class="fa fa-eye-slash "></i></a>
            <?php }?>
            <?php
                if( $post_status == 'expired' ){ 
            ...
            if($paid_submission_status=='membership'){
                ...
                if ( intval(get_post_meta($post_id, 'prop_featured', true))==1){
                     print '<span class="label label-success">'.__('Sublease is featured','wpestate').'</span>';       
                }
                else{
                ...
                //   print'<span data-original-title="'.__('Resend for approval','wpestate').'" class="dashboad-tooltip resend_pending" data-listingid="'.$post_id.'"><i class="fa fa-upload deleteprop"></i></span>';   
                    ...
                    if($paid_submission_status=='per listing'){
                        ...
                        if($pay_status!='paid' ){
                            ...
                            if ( intval(get_post_meta($post_id, 'prop_featured', true))==1){
                                print '<span class="label label-success featured_label">'.__('Sublease is featured','wpestate').'</span>';  
                            }else{
                                 print' 
    ...
    <div class="statistics_wrapper">
        <div class="statistics_wrapper_total_views">
            <?php _e('Total number of views:','wpestate'); echo $no_views; ?>
        </div>
