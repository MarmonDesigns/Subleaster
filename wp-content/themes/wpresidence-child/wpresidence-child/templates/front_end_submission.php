...
if( !isset( $_GET['listing_edit'] ) && $paid_submission_status == 'membership' && $remaining_listings != -1 && $remaining_listings < 1 ) {
    print '<div class="user_profile_div"><h4>'.__('Your current package doesn\'t let you publish more subleases! You need to upgrade your membership.','wpestate' ).'</h4></div>';
}else{
...
<form id="new_post" name="new_post" method="post" action="" enctype="multipart/form-data" class="add-estate">
    ...
    <input type="hidden" name="action" value="<?php print $action;?>">
        <div class="submit_form_row">
            ...
            ?>
               <input type="submit" class="wpb_button  wpb_btn-info wpb_btn-large" id="form_submit_1" value="<?php _e('ADD SUBLEASE', 'wpestate') ?>" />
            <?php
            }
            ?>
        </div>  
